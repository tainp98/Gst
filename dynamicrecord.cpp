...
// videotestsrc ! tee name=t t. ! x264enc ! mp4mux ! filesink location=/home/jason/Downloads/0.mp4 t. ! videoconvert ! autovideosink

static GMainLoop *loop;
static GstElement *pipeline, *src, *caps_filter, *tee, *encoder, *muxer, *filesink, *videoconvert, *videosink, *queue_record, *queue_display;
static GstBus *bus;
...
//        templ = gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(tee), "src_%u");
        ...
        gst_bin_add_many(GST_BIN(pipeline), queue_record, encoder, muxer, filesink, NULL);
...
int main(int argc, char *argv[])
{
        ...
        //videosink = gst_element_factory_make("nveglglessink", NULL);

        //caps = gst_caps_from_string("video/x-raw, width=(int)640, height=(int)360");
        //g_object_set(caps_filter, "caps", caps, NULL);

        if (!pipeline || !src || !tee || !queue_display || !videoconvert || !videosink) {
        ...
        if (!gst_element_link_many(src, tee, NULL)
                || !gst_element_link_many(queue_display, videoconvert, videosink, NULL)) {
                g_error("Failed to link elements");
                return -2;
        }

        GstPad *teepad1, *videopad;
        teepad1 = gst_element_get_request_pad(tee, "src_%u");
        videopad = gst_element_get_static_pad(queue_display, "sink");
        gst_pad_link(teepad1, videopad);
